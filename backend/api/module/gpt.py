...
with open('/cloudide/workspace/Django/backend/api/module/cookies_headers.json', 'r') as file:
    data = json.load(file)
...
def changeIdref(headers, chat_id):
    parsed_url = urllib.parse.urlparse(headers["referer"])
    query_params = urllib.parse.parse_qs(parsed_url.query)

    query_params["id"] = [chat_id]

    # Rekonstruksi URL referer
    new_query = urllib.parse.urlencode(query_params, doseq=True)
    new_referer = urllib.parse.urlunparse(parsed_url._replace(query=new_query))
...
def get_response_value(prompt):
    try:
        ...
        while coba <= 3:
            ...
            # new_referer = changeIdref(headers=headers, chat_id=chat_id)
...
#     result = getAnswer("apa itu manusia?")
#     print(result)
